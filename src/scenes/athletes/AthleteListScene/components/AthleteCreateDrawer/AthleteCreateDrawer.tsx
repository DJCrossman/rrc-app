...
interface AthleteCreateDrawerProps {
	isOpen: boolean;
	onClose: () => void;
	onSubmit: (data: CreateAthlete) => Promise<void> | void;
}

export const AthleteCreateDrawer = ({
	isOpen,
	onClose,
	onSubmit,
}: AthleteCreateDrawerProps) => {
		...
		>
			<DrawerContent>
				<DrawerHeader className="flex flex-row items-center justify-between border-b">
					<div>
						<DrawerTitle>Add Athlete</DrawerTitle>
						<DrawerDescription>Create a new athlete profile</DrawerDescription>
					</div>
				...
				<div className="flex-1 overflow-y-auto p-6">
					<AthleteForm onSubmit={handleSubmit} onCancel={onClose} />
				</div>
			</DrawerContent>
		</Drawer>
	);
