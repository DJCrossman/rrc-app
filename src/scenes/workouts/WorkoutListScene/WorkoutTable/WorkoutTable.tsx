...
export function WorkoutTable({ data }: WorkoutTableProps) {
  const [currentWeek, setCurrentWeek] = useState(DateTime.now().startOf('week').minus({ days: 1 }));
  ...
  const workoutsByDay = currentInterval
    ...
    .map((dayInterval) => {
      ...
      const morningWorkouts = data.filter(
        (workout) =>
          dayStart && DateTime.fromISO(workout.startDate) >= dayStart &&
          DateTime.fromISO(workout.startDate) < dayStart.plus({ hours: 12 })
      ).map((workout) => ({
        ...workout,
        timeFrame: workout.duration ? Interval.fromDateTimes(DateTime.fromISO(workout.startDate), DateTime.fromISO(workout.startDate).plus({ milliseconds: workout.duration })).toLocaleString(DateTime.TIME_SIMPLE) : 'N/A'
      }));

      const eveningWorkouts = data.filter(
        (workout) =>
          dayStart && dayEnd && DateTime.fromISO(workout.startDate) >= dayStart.plus({ hours: 12 }) &&
  ...
  return (
    <div className="w-full flex-col justify-start gap-6">
      <div className="flex justify-between items-center py-6 px-4 xl:px-6">
        ...
        <h2>
          {currentInterval.toLocaleString(DateTime.DATE_MED)}
        </h2>
        <button onClick={() => {
        ...
        {workoutsByDay.map(({ day, morningWorkouts, eveningWorkouts }) => (
          <div key={day?.toISO()} className="xl:w-1/7 mb-6">
            <h3 className="text-lg font-bold text-center">
              <span className='block uppercase'>{day?.toLocaleString({ weekday: 'short' })}</span>
              <span className='block text-lg'>{day?.toLocaleString({ day: 'numeric' })}</span>
            </h3>
            <div className="mt-4 xl:min-h-[200px]">
              <h4 className="font-semibold mb-2">Morning</h4>
              {!!morningWorkouts.length && morningWorkouts.map((workout) => (
            ...
            <div className="mt-4 xl:min-h-[200px]">
              ...
              {!!eveningWorkouts.length && eveningWorkouts.map((workout) => (
                <WorkoutItem key={workout.id} workout={workout} />
              ))}
...
const WorkoutItem = ({ workout }: { workout: Workout & { timeFrame: string } }) => {
  return (
    <Card className='my-2 p-0'>
      <a href={`/workouts/${workout.id}`} className='h-full w-full p-2'>
        {workout.description.split(';').map((line, index) => (
          <span key={line} className={cn('block', index > 0 ? 'text-sm' : 'font-semibold')}>{line.trim()}</span>
        ))}
