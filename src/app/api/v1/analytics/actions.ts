...
import { mapToAthleteDto } from "../athletes/utils";
...
export const getAnalytics = async () => {
	...
	// ── Meters time series (last 30 days) ──────────────────────────────────────
	...
	const leaderboard = allAthletes.map((row) => {
		const athlete = mapToAthleteDto(row);
		...
		return {
			...
			activeMembership: athlete.activeMembership,
			meters,
			points: 0,
...
export type AnalyticMetrics = Awaited<
	ReturnType<typeof getAnalytics>
>["analyticMetrics"];
export type MetersTimeSeries = Awaited<
	ReturnType<typeof getAnalytics>
>["metersTimeSeries"];
export type Leaderboard = Awaited<
	ReturnType<typeof getAnalytics>
>["leaderboard"];
